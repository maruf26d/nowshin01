...
function skipBoot() {
    ...
    document.body.style.background = "#fff url(src/wallpaper/img18.png) center center/cover no-repeat";
}
// skipBoot();
...
function passwordPhase() {
    ...
    function askPassword() {
        ...
        passwordField.addEventListener("keyup", event => {
            if (passwordField.value == "ami ekta goru") {
                    ...
                    setTimeout(_ => {
                        ...
                        if (window.navigator.userAgent.indexOf("Firefox") > -1) {
                            if (CSS.supports("(backdrop-filter:blur(20px))") != true) {
                                notify("src/icons/mar.jpg", "MARUF", "Hello Firefox User", "To get full UI experience please go to \"<span style=user-select:all>about:config</span>\" and search for \"layout.css.backdrop-filter.enabled\" and set it to \"true\"")
                            }
                        }
                    }, 2e3)
                    setTimeout(notify, 10e3, "src/icons/mar.jpg", "MARUF", "A Shayari Just for You 💖", "Janamdin ki dheron shubhkamnayein, Khushiyon se bhare rahe tere sapne saare,\n Har pal muskuraati rahe zindagi teri, Happy Birthday Nowshin, tu sada rahe pyari!💖")

                }
                if (getCookie("cookiesAccepted") == "") {
                    notify("src/icons/mar.jpg", "MARUF", "Explore this little gift from me…", "go through each file carefully, something truly special is waiting just for you. 💖", "Accept", `setCookie('cookiesAccepted','1');notificationClose('#n${nId}')`);
                }
